...
2. **Incrementing Counts**:
   ...
   2. If not, it initializes $counter\[x\] = 0$ before incrementing, so every newly encountered item is seamlessly added with the appropriate count.

3. **Retrieving Counts and Most Common Elements**:
   1. Accessing the current count $counter\[x\]$ simply looks up the value of $x$ in the internal dictionary. If $x$ is absent, a default of 0 is returned.
   2. The method
      $word\\_counter.most\\_common(k)$
      performs an internal sort (or partial sort) of the dictionary items by their counts in descending order, returning the top $k$ most frequent elements as *element*, *count* pairs.
...
1. **Initialize the algorithm**:
   ...
   2. Create an empty map (dictionary) $counter = \emptyset$ that stores $(item$ &rarr; $count)$.

2. **Process each incoming item $x$**:
   1. Generate a random number $r$ in the range $[0,1)$.
   2. If $r < p$:
      1. Increment $counter\[x\]$ by 1. (If $x$ is not yet in $counter$, initialize $counter\[x\] = 0$ before incrementing.)
...
3. **Frequency Estimation**:
   1. For an item $x$, the estimated count is given by:
      $\widehat{f}(x) = counter\[x\] \times \biggl\lfloor \tfrac{1}{p} \biggr\rfloor.$
...
- The method is *unbiased* in expectation, but it can exhibit high if $p$ is small.
...
Define the random variable $X_i$ for the $i^{th}$ occurrence of the word as:
...
Since each occurrence is included independently with probability $p$, $X_i$ follows a bernoulli distribution:
...
1. **Initialize the algorithm**:
   ...
   2. Create an empty map (dictionary) $counter = \emptyset$, which stores $(item$ &rarr; $count)$.
   3. Create a min-heap $H$ for the pairs $(count, item)$.

2. **Process each incoming item $x$ in the data stream**:
   1. **Case 1:** If $x$ is already in $counter$:
      1. Increment its count: $counter\[x\] = counter\[x\] + 1$.
      2. Update the corresponding pair $(count, x)$ in the min-heap $H$.
   2. **Case 2:** If $x$ is *not* in $counter$ and $|counter| < k$:
      1. Set $counter\[x\] = 1$.
      2. Insert $(1, x)$ into the min-heap $H$.
   3. **Case 3:** If $x$ is *not* in $counter$ and $|counter| = k$:
      ...
      3. Insert $x$ in $counter$ with $counter\[x\] = min\\_count + 1$.
      4. Push $(min\\_count + 1, x)$ into $H$.

3. **Output**:
   1. After processing the entire stream, the map $counter$ holds the items and their approximate counts.
...
![Comparison of Precision between Fixed Probability Count and Exact Count for Top and Bottom Frequent Words](fixed_probability.png)  
...
Looking at the results for each method, the **Exact Count** correctly identified two terms (*king* &rarr; *rei*, *rous* &rarr; *grande*), the **Fixed Probability** method correctly identified one term (*monarch* &rarr; *rei*), and the **Space Saving** method also correctly identified one term (*spread* &rarr; *largos*, *lo* &rarr; *vereis/olhando*). It is possible that not all correct terms were captured due to translation nuances.

---

## Conclusion

This study meticulously evaluated the efficacy of three word counting algorithms—**Exact Count**, **Fixed Probability Sampling**, and the **Space Saving** method—in identifying the most and least frequent words within large text datasets. Utilizing a diverse collection of texts from [gutenberg](https://www.gutenberg.org/), our experiments underscored the strengths and limitations inherent to each method.
...
Conversely, the *Space Saving* method showcased robust memory efficiency by maintaining a fixed number of $(*word*, *count*)$ pairs. This approach effectively identified frequent words while dynamically managing less frequent ones through strategic evictions. Our analysis revealed that setting the parameter $k$ to approximately 2250 strikes an optimal balance between ranking accuracy and computational resource utilization. In cases where *k* exceeds the number of unique words, the Space Saving method aligns closely with Exact Count results.
